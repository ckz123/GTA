...
# IMDB
optimizer = optim.Adam(list(tplink.parameters()) + \
                       list(mymlp.parameters()), lr=1e-2)#, weight_decay=5e-5)
...
def listen_for_input():
    global save_flag
    while True:
        input_value = input()  # 监听用户输入
        if input_value == '':  # 用户按下 Enter 键
            save_flag = True  # 设置标志，标识需要保存模型
import threading
...
for i in range(epoch):
    ...
    # 在每个 epoch 完成后检查是否需要保存模型
    if save_flag:
        save_model(tplink, 'tplink_model.pth')
        save_model(mymlp, 'mymlp_model.pth')
        save_flag = False  # 重置标志，继续训练

    # 等待一定时间模拟训练进程
    # time.sleep(3)  # 假设训练过程需要一些时间，调整为你的实际情况
